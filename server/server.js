...
app.post('/api/farmers/register', (req, res) => {
  try {
    ...
    const newFarmer = {
      ...
      name: cleanName,
      phone: cleanPhone,
      aadhar: cleanOptional(aadhar ?? aadharNumber),
      address: cleanOptional(address),
      district: cleanOptional(district) || 'Sangareddy / Medak',
      mandal: cleanOptional(mandal) || 'Patancheru',
      bankAccount: cleanOptional(bankAccount ?? accountNumber),
      ifscCode: cleanOptional(ifscCode) || 'SBIN0020145',
      upi: cleanOptional(upi),
      createdAt: new Date()
    };
